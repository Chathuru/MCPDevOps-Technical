...
resource "aws_security_group" "eks_cluster" {
  name                    = join("-", [var.eks_cluster_name, var.cluster_sg_name])
...
resource "aws_iam_role" "eks_nodes" {
  name                    = join("-", [var.eks_cluster_name, var.environment, "NG-ROLE"])
...
resource "aws_eks_node_group" "public" {
  ...
  node_group_name     = join("-", [var.eks_cluster_name, "NG-PUBLIC"])
  ...
  tags = {
    Name              = join("-", [var.eks_cluster_name, "NG-PUBLIC"])
  }
...
resource "aws_eks_node_group" "private" {
  cluster_name        = aws_eks_cluster.eks.name
  node_group_name     = join("-", [var.eks_cluster_name, "NG-PRIVATE"])
  node_role_arn       = aws_iam_role.eks_nodes.arn
  ...
  tags = {
    Name              = join("-", [var.eks_cluster_name, "NG-PRIVATE"])
  }
# Ensure that IAM Role permissions are created before and deleted after EKS Node Group handling.
  # Otherwise, EKS will not be able to properly delete EC2 Instances and Elastic Network Interfaces.
  depends_on = [
